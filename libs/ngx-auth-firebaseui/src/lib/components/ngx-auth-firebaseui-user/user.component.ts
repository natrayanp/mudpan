...
export class UserComponent {
    ...
    if (this.updateFormGroup.dirty) {
      ...
      try {
        ...
        this.authProcess.showToast(
          error instanceof Error ? (error && error.message ? error.message : '') : error as string
        );
        console.error(error);
      }
  ...
  async deleteAccount() {
    try {
      ...
      this.authProcess.showToast(
        `Error occurred while deleting your account: ${error instanceof Error ? error.message : ''}`
      );
    }
  }
      ...
      map((currentUser) => {
        ...
        const updateFormGroup = new FormGroup({
          name: this.updateNameFormControl = new FormControl(
            { value: currentUser?currentUser.displayName:'', disabled: this.editMode },
            [
              Validators.required,
          ...
          email: this.updateEmailFormControl = new FormControl(
            { value: currentUser?currentUser.email:'', disabled: this.editMode },
            [Validators.required, Validators.pattern(EMAIL_REGEX)]
          ),

          phoneNumber: this.updatePhoneNumberFormControl = new FormControl(
            { value: currentUser?currentUser.phoneNumber:'', disabled: this.editMode },
