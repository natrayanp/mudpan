...
export class AuthProcessService implements ISignInProcess, ISignUpProcess {
  ...
  public async signInWith(provider: AuthProvider, credentials?: ICredentials) {
    try {
      ...
      switch (provider) {
        ...
        default:
          throw new Error(
            `${provider} is not available as auth provider`
          );
      }
  ...
  public async signUp(displayName: string, credentials: ICredentials) {
    try {
      ...
      await this.updateProfile(displayName, user ? user.photoURL : "");
  ...
  async sendNewVerificationEmail(): Promise<void | never> {
    ...
    return this._user$.value.sendEmailVerification();    
  }
   ...
   * @returns -
   */
  public updateProfile(name: string, photoURL: string | null): Promise<void> {
    
    return this.afa.currentUser.then((user: firebase.User | null) => {
        if(user){
          if (!photoURL) {
  ...
  public parseUserInfo(user: firebase.User | null): firebase.UserInfo {
    if(user){
    return {
      uid: user.uid,
      ...
      photoURL: user.photoURL,
      providerId:
        user.providerData.length > 0 ? (user.providerData[0] ? (user.providerData[0].providerId) : "" ): "",
    };
  } else {
  ...
  async handleSuccess(userCredential: UserCredential) {
    ...
    if (this.config.toastMessageOnAuthSuccess) {
      const fallbackMessage = `Hello ${        
        userCredential.user ? (userCredential.user.displayName ? userCredential.user.displayName : "") : ""
      }!`;
